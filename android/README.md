...
- Home screen widgets, one per Today card: **Right now**, **Your day**, **Schedule**, **Tasks**, and **Habits**. They draw from a week-long snapshot the app saves whenever your plan changes (in the app's colors), so they stay current while the app is closed. Tapping a task's checkbox or a habit's circle updates the widget at once and is applied in the app the next time it runs; the + buttons open the app to add one (`PanelWidgets.java`, `WidgetDraw.java`).
